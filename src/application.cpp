    ...
    : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QStringLiteral("Pisces Settings"));
    ...
    if (!QDBusConnection::sessionBus().registerService("com.pisces.SettingsUI")) {
        QDBusInterface iface("com.pisces.SettingsUI", "/SettingsUI", "com.pisces.SettingsUI", QDBusConnection::sessionBus());
        if (iface.isValid())
    ...
    // QML
    const char *uri = "Pisces.Settings";
    ...
    // Translations
    QLocale locale;
    QString qmFilePath = QString("%1/%2.qm").arg("/usr/share/pisces-settings/translations/").arg(locale.name());
